...
export default function request(
 ...
 const userDetail = xCookie.get('userData') ? JSON.parse(xCookie.get('userData')) : { loginName: '' }
  ...
  return fetch(apiUrl, newOptions)
    ...
    .then(response => {
      let responseAuthCookie = response.headers.get('authentication-info')
      if (responseAuthCookie) {
        xCookie.set('authentication', responseAuthCookie);
      }
      return response.json()
    })
    .then(data => {
