...
export default class TKTradetypeMoreSelect extends Component {
  constructor(props) {
    super(props)
  }
    ...
    return(
      <Spin spinning={loading}>
        <Select {...this.props} allowClear={true} mode="multiple">
          {
            options.map( (item) => {
              return <Option value={item.transMapId.toString()} key={item.transMapId.toString()} >{item.transSubname} </Option>
            })
          }
        </Select>
      </Spin>
