...
import { Row, Col, Card, Form, Input, Select, DatePicker, Button, Table, Icon, Divider, Drawer, Popconfirm, Modal, message } from 'antd';
import {formatTime, responseMsg, getObjStatus, changeTime} from '@/utils/utils';
import TKPhposSelect from '@/components/TKPhposSelect/phposSearch';
...
@connect(({ phpos, loading }) => ({
  ...
  resultDetailList: phpos.detailListData,
...
class EditPage extends Component {
  ...
  handleSubmit = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) return;
      const { langCode, langName, langUrl } = fieldsValue
      const { detailList } = this.state
      
      if(detailList.length == 0){
        message.error('请先添加终端！');
        return;
      }

      const values = {
        otherType: 0, //0：报备；1：修改
  ...
  getPhposData = (data) => {
    ...
    const newData = [...this.state.detailList];
    const target = newData.filter(item => data.id === item.id)[0];
    if (!target) {
      newData.push({
        id: data.id,
        posNo: data.posNo,
        hostMapId: detailData.hostMapId,
        hostMerNo: detailData.hostMerNo,
        hostPosNo: '',
        phposBatch: '',
        phposTrace: '',
      });
  ...
  render() {
    const { detailData } = this.props;
    const { detailList,phposModal } = this.state;
      ...
      <Fragment>
        <div className={styles.editFormItem}>
          <Form layout='vertical' onSubmit={this.handleSubmit}>
            ...
            <TKPhposSelect
              getmore={(data) => this.getPhposData(data, 0)}
