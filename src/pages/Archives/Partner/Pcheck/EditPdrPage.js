...
class EditPdrPage extends Component {
  ...
  mdrFixFeeRender = (key, label, ischoose=true) => {
    return label
    // return(
    //   <Checkbox onChange={(e)=>this.valueChanged(key, e.target.value)} checked={ischoose}>{label}</Checkbox>
    // )
  }
      ...
      if (isAdd) {
        for (let i = 0; i < 2; i++) {
          payTypeRender.push(<Timeline.Item key={num}>
            ...
            <FormItem >
              {getFieldDecorator(`pdrList[${num}]['paytypeMapId\']`, Object.assign({}, {}, {initialValue: detailData && item.paytypeMapId ? item.paytypeMapId : item.paytypeId}))
              (
            ...
            <FormItem >
              {getFieldDecorator(`pdrList[${num}]['cardCd\']`, Object.assign({}, {}, {initialValue: detailData && item.cardCd != undefined ? item.cardCd : i}))
              (
                <Input type='hidden' placeholder='修改订单编号'/>
              )}
            </FormItem>
        ...
        payTypeRender.push(<Timeline.Item key={num}>
          ...
          <FormItem >
            {getFieldDecorator(`pdrList[${num}]['cardCd\']`, Object.assign({}, {}, {initialValue: detailData && item.cardCd != undefined ? item.cardCd : 9}))
            (
              <Input type='hidden' placeholder='修改订单编号'/>
            )}
          </FormItem>
