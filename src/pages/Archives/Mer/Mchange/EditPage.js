...
@connect(({ merchantchange, loading }) => ({
  detailRes: merchantchange.detailRes,
  detailResLoading: loading.effects['merchantchange/detail'] ? true : false
}))
...
class EditPage extends Component {
  ...
  addAction = () => {
    ...
    if(detailData) {
      subData = {
        ...
        mcerttmp: submitData[1],
        mbanktmp: submitData[2],
        mdrtmpsList: submitData[3]
      }
    }else{
      subData = {
  ...
  render() {
    ...
    const { onClose, onReturnList, detailData } = this.props;
    return (
      <div>
        ...
        <div style={{display: nextFlag==1 ? 'block' : 'none'}}>
          <EditCertPage
            checkType={detailData.checkType}
