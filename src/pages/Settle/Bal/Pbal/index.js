...
@connect(({ pbal, loading }) => ({
  result: pbal.data,
  resultLoading: loading.effects['pbal/search'] ? true : false
}))
@Form.create()

export default class Pbal extends Component {
  state = {
    detailData: null,
    detailVisible: false,
    editVisible: false,
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) return;
      const { partnerMapId } = fieldsValue
      const values = {
        partnerMapId
      };
    ...
    return (
      <div>
        ...
        <Card key={'b'}
          ...
          title={<span className={styles.tableTitle}><Icon type="appstore-o" /> 渠道结算管理
