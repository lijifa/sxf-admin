...
const namespace = 'htbill';
@connect(({ htbill, loading }) => ({
  result: htbill.data,
  resultLoading: loading.effects['htbill/search'] ? true : false
}))
@Form.create()
...
export default class Htbill extends Component {
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) return;
      const { hostName, date } = fieldsValue
      const values = {
        hostName,
  ...
  renderSearchForm() {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline" className="search">
        <div className="kuai">
          <span className="span">通道名称 :</span>
          <FormItem className="inputW160">
            {getFieldDecorator('hostName')(
              <TKHostSelect placeholder='请选择通道名称'/>
            )}
          </FormItem>
        </div>
    ...
    if (list) {
      tableData = list.map((item, idx) => {
        ...
        return {
          ...
          ptMdrHostFee_str: settleCoinCode + ':' + covertMoney2Yuan(ptMdrHostFee),
          ptFeeIss_str: settleCoinCode + ':' + covertMoney2Yuan(ptFeeIss),
          ptFeeBrand_str: settleCoinCode + ':' + covertMoney2Yuan(ptFeeBrand),
          ptFeeNet_str: settleCoinCode + ':' + covertMoney2Yuan(ptFeeNet),
    ...
    return (
      <div>
        ...
        <Card key={'b'}
          ...
          title={<span className={styles.tableTitle}><Icon type="appstore-o" /> 通道日对账单
